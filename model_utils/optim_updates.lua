...
-- Project: neuraltalk2 https://github.com/karpathy/neuraltalk2
-- Slightly modified by Xiao Lin for initial values of rmsprop.
...
function rmsprop(x, dx, state)
  ...
  local learningRate = state.learningRate or 1e-2;
...
function adam(x, dx, state)
  ...
  local epsilon = state.epsilon or 1e-8
  local lr = state.learningRate or 1e-2;
